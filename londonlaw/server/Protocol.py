...
class LLawServerProtocol(basic.LineOnlyReceiver):
   ...
   def cmd_ban_admin(self, tag, args):
      if args == []:
         ...
         try:
            ...
            raise DeniedCommand(_("Player name not found."))
            ...
            try:
               ...
               if client._game == g:
                  ...
                  client.sendUntagged("ejected", 
                        _("The server admin ejected you from this game."))
            except KeyError:
               pass
   ...
   def cmd_disconnect_admin(self, tag, args):
      if args == []:
         ...
         try:
            ...
            raise DeniedCommand(_("No such user is connected."))
         client.transport.loseConnection()
         self.sendOk(tag)
   ...
   def cmd_doublemove_playing(self, tag, args):
      ...
      if args[0] == pawn.getName().lower():
         if self._username == pawn.getPlayer() and self._game.isLegalMove(
            ...
            self.sendNo(tag, _("That move is illegal."))
      else:
         self.sendNo(tag, _("It is not that pawn's turn."))
   ...
   def cmd_eject_admin(self, tag, args):
      if len(args) < 2:
         ...
         if g.getStatus() == GAMESTATUS_NEW or g.getStatus() == GAMESTATUS_COMPLETE:
            ...
            raise DeniedCommand(_("Game is in progress."))
      ...
      if args == []:
         ...
         try:
            ...
            try:
               ...
               raise DeniedCommand(_("Unsupported language code."))
   ...
   def cmd_listai_joined(self, tag, args):
      if len(args) > 0:
         if args[0] == "Mr. X":
            ...
            raise DeniedCommand(_("Unrecognized team name."))
      else:
         raise IllegalCommand(_("Insufficient arguments."))
   ...
   def cmd_login_default(self, tag, args):
      raise DeniedCommand(_("Already logged in."))
   ...
   def cmd_login_init(self, tag, args):
      raise DeniedCommand(_("Protocol version not verified."))
   ...
   def cmd_protocol_default(self, tag, args):
      raise DeniedCommand(_("Protocol version already verified."))
   ...
   def cmd_protocol_init(self, tag, args):
      ...
      if len(args) > 0:
         if args[0] == PROTOCOL_VERSION:
            ...
            raise IllegalCommand(_("Incompatible protocol version."))
         else:
            raise IllegalCommand(_("Unrecognized protocol string."))
      else:
         raise IllegalCommand(_("Insufficient arguments."))
   ...
   def lineReceived(self, line):
      try:
         ...
         if len(tokens) == 0:
            raise ServerError(_("Insufficient arguments."))
         ...
         if f is None:
            self.sendBad(tag, _("Unrecognized command."))
            return
...
class ProtocolGameListener:
   ...
   def gameOverEvade(self, winningTeam):
      ...
      client.sendUntagged("gameover", winningTeam.getName(), 
            _("Mr. X successfully evaded the detectives for 24 turns.  Mr. X wins!"))

   def gameOverStuck(self, winningTeam):
      client = GameRegistry.registry.getClient(self._username)
      client_state = "endgame"
      client.sendUntagged("gameover", winningTeam.getName(), 
            _("None of the detectives are able to move.  Mr. X wins!"))

   def gameOverCaught(self, winningTeam, detective):
      client = GameRegistry.registry.getClient(self._username)
      client_state = "endgame"
      client.sendUntagged("gameover", winningTeam.getName(), 
         (_("Mr. X was caught by the %(color)s Detective at location %(number)d.  The detectives win!") % \
   ...
   def pawnMove(self, game, pawn, *moves):
      if len(moves) == 1:
         ...
         raise ServerError(_("Invalid number of moves."))
